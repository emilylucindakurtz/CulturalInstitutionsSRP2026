...
```{r}
bls_data_M13 <- bls_data %>% 
  filter(period == "M13", # Just annual
         measure_text == "unemployment rate")
  
```
```{r}
bls_data_M13  <- bls_data_M13 %>% 
...
2 options...
1) df <- df %>% 
  mutate(column_name = if_else(column_name == "old_value", "new_value", column_name))
  
  library(dplyr)

2) # Update multiple specific statuses at once
df <- df %>% 
  mutate(status = case_when(
    status == "Pending"  ~ "Processing",
    status == "Rejected" ~ "Declined",
    status == "Hold"     ~ "Paused",
    TRUE                 ~ status  # Keeps everything else exactly the same
  ))

```{r}
# Fixing the issues with Alaska areas: Yakutat, Anchorage, Sitka, Wrangell, Juneau

#Could def do this quicker w loop but already wrote this so
bls_data_M13 <- bls_data_M13 %>% 
  mutate(NAMELSAD = case_when(
    NAMELSAD == "Anchorage Borough/municipality"~ "Anchorage Municipality",
    NAMELSAD == "Juneau Borough/city" ~ "Juneau City and Borough",
    NAMELSAD == "Sitka Borough/city" ~ "Sitka City and Borough",
    NAMELSAD == "Wrangell Borough/city" ~ "Wrangell City and Borough",
    NAMELSAD == "Yakutat Borough/city" ~ "Yakutat City and Borough",
    TRUE ~ NAMELSAD # Keeps everything else exactly the same
  ))
```
```{r}
bls_data_M13 <- bls_data_M13 %>% 
  mutate(unemployment_percent = value*100) %>%  # ^ value is the unemployment
  select(-value)
```
```{r}
write_csv(bls_data_M13, file = "annual_bls_LAUS_1990_2025.csv")
```

```{r}
#Joining to shapefile
mapping_data_bls <- counties_sf %>% 
  left_join(bls_data_M13, by = c("NAMELSAD", "STUSPS")) 
```
MAPPING TEST HERE BLS M13 UPDATE
...
pal <- colorQuantile(
  ...
  palette = "Spectral",
  domain = mapping_data_bls$unemployment_percent,
  n = 10,
  na.color = "grey",
  reverse = TRUE
)
...
2025
```{r}
# Choosing the year to map
to_map <- mapping_data_bls %>% 
  filter(year == 2025)
```
...
leaflet(to_map) %>% 
  ...
  addPolygons(
    fillColor = ~pal(unemployment_percent),
    fillOpacity = 1,
  ...
  addLegend(
    pal = pal,
    value = mapping_data_bls$unemployment_percent, # same as values  
...
pal_bls_breaks <- quantile(mapping_data_bls$unemployment_percent, probs = seq(0, 1, length.out = 11), na.rm = TRUE)
```
```{r}
...
```{r}
#| eval: false
# Prepping to join data to shapefile
county_equivs <- paste("County", "Planning Region", "Borough", "Census Area", "/municipality", "Municipality", "/city", "Parish", sep = "|")

bls_data_M13  <- bls_data_M13 %>% 
  mutate(NAMELSAD = str_remove(area_text, ",.*"), #before comma
         STUSPS = str_trim(str_replace(area_text, "^.*,","")), #after comma
         NAME = str_squish(str_remove_all(NAMELSAD, county_equivs))) #Fixing the issues with Alaska areas:Yakutat, Anchorage, Sitka, Wrangell, Juneau
